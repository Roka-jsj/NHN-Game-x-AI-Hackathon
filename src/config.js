...
//
// **실측으로 골랐다** (격리 트리에서 봇 10종 × 사령관 5 × 씨앗 16 = 80판씩):
//   지배 11건 → 0건 · 합산효율 최대최소비 6.02배 → 1.04배
//   도배 봇 최고 승률 64%(거인 도배) → 21% · 상성 봇 29% → 43%
//   격차 −35%p → +18%p (아래 COUNTER_STRONG 주석에 이어진다)
...
//
// **1.75 → 3.5. 이 인상은 예전에 한 번 거부됐다. 왜 이번엔 다른가 —**
...
//     배수 1.75 → 격차  2%p      배수 2.5 → 18%p
//     배수 3.0  → 격차 20%p      배수 3.5 → 26%p
//   즉 **배수가 값을 갖게 된 것은 가격을 고친 뒤부터다.**
// 값은 **평가기 [10] 로 직접 골랐다** (사령관 다섯이 상대. 이게 합격 판정자다):
//     배수 2.5  상성 2/5(40%) 도배궁 1/5(20%) → 격차 20%p  기지비율차 +0.369  ✖
//     배수 3.5  상성 4/5(80%) 도배궁 1/5(20%) → 격차 60%p  기지비율차 +1.015  ○
//   3.5 에서 상성 봇이 S0~S3 를 이기고 S4 만 진다. 승률은 5표본이라 거칠지만
//   연속값인 기지비율 격차가 0.369 → 1.015 로 같이 움직여 서로를 받쳐 준다.
export const COUNTER_STRONG = 3.5;
