...
//      두면 판별력이 0.31 → 0.04 로 무너졌다. 상성 유닛을 아무리 많이 뽑으라고
//      지시해도 **뽑을 돈이 없으면 화면에 안 나온다.**
//      구성은 방향이고 수입은 크기다. 둘 다 있어야 지시가 병력이 된다.
//   2) **그렇다고 둘 다 켜면 오히려 나빠진다** (0.26 vs 한쪽만 켰을 때 0.35).
//      진화 가속은 ERA_BASE_HP_MUL 때문에 **적 기지 체력을 4~6배로 불린다** —
//      계약 §5.5 가 금지한 "기지를 그냥 크게 만들기"가 뒷문으로 들어온다.
//      그래서 진화 채널을 끄고(FOCUS_ERA=0) 수입 채널만 남겼다.
//
//   측정 조건을 같이 적어 둔다 — 조건이 바뀌면 계수도 다시 재야 한다:
//   봇 9종 × 스테이지 5 × 특성 유무 2 = 90판, data/chunks.json 은 authored 판본.
//   같은 스윕을 placeholder 라이브러리로 돌리면 판별력이 0.35 까지 올라간다.
//   **웨이브 라이브러리가 바뀌면 이 계수의 효과도 같이 바뀐다.**
...
export class Director {
  ...
  selectChunk(tags) {
    ...
    //
    // **보폭은 반드시 1이어야 한다.** 예전에 `lastChunk * 7` 이었는데, 칸당
    // 변형이 14개라 `gcd(7,14) = 7` 때문에 `7·ci mod 14 ∈ {0, 7}` 두 값밖에
    // 안 나왔다. `switches` 가 고정된 동안 **14개 중 2개만 도달 가능**이라,
    // 구운 웨이브 350개 중 실효 50개만 쓰이고 있었다.
    // 실측: 70초 한 판에서 쓰인 웨이브가 350개 중 7개, 칸마다 `-1`·`-8` 변형만.
    //
    // 7 을 5 로 바꾸는 식의 수정은 틀렸다. **어떤 고정 보폭도 안전하지 않다** —
    // 스윕 결과 7은 n=14에서, 5는 n=10에서, 3은 n=12에서 각각 무너진다.
    // 1 만이 모든 n 과 서로소다. 변형 간 다양성은 `switches` 오프셋이 맡는다.
    const pick = this.candidates[Math.abs(this.lastChunk + this.switches * 3) % n];
