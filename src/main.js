...
// ── 시도했다가 되돌린 것: game.setZodiacSeed(Date.now()) ──
// "신규 세션의 첫 별자리가 항상 같다"(zodiacSeed 기본값 0, NAN 2026 심사위원단
// 지적)를 고치려 했다. 그런데 tools/evaluate.mjs 는 game.js 를 직접 import하지
// **않는다** — playwright 로 이 index.html/main.js 를 실제 브라우저에 그대로
// 띄운다(evaluate.mjs 497번째 줄 근처). 즉 여기서 나가는 모든 줄을 평가기의
// 봇도 똑같이 거친다. 그리고 config.js 309번째 줄 주석이 이미 실측해 뒀듯
// **씨앗 0 은 상성 봇에게 최상단 표본이다**(6벌 평균보다 유리) — 격차 40%p
// 게이트 자체가 그 표본 위에서 정의돼 있다. Date.now() 로 씨앗을 흔들자
// 격리 트리 재측정에서 상성−도배 격차가 +40%p → **−20%p**로 뒤집혔고
// 상성 삼각형 동금도 6→5로 깨졌다 — 화면용 장식이라고 생각했던 값이 실은
// 전투 배수(사거리·쿨다운 등, ZOD_* 상수)를 통해 판정에 실제로 개입하고
// 있었다. 평가기에 test-mode 플래그(예: ?seed=0 쿼리 파싱)를 새로 만들지
// 않는 한 여기서 안전하게 고칠 수 없다 — 그건 QA 게이트 자체를 건드리는
// 별도 작업이라 이번 라운드 범위 밖이다. docs/game-review.md 부록에 남긴다.
game.reset();
...
function resize() {
  ...
  // 눕힌 캔버스를 합성 레이어로 승격시키는 힌트.
  // **측정이 결론을 못 냈다는 것을 적어 둔다.** 버퍼 크기는 세로·가로가 똑같고
  // (1386×780) JS 드로우 시간도 같았다 (p50 0.7~0.8ms). 프레임 간격은 눕혔을 때
  // 나빠 보였지만, 같은 설정을 반복하니 가로 쪽도 8→65/300 으로 튀었다 —
  // 이 컨테이너에서 여러 하네스가 동시에 도는 잡음이지 회전 비용이 아니다.
  // 그래서 이 한 줄은 "실측으로 이겼다"가 아니라 공짜인 보험으로 둔다.
