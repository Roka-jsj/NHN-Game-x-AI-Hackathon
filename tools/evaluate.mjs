...
// 18 → 32 분. **실제 소요가 아니라 추정기가 병목이었다.**
// 도배 봇을 여섯 종으로 넓히자(격차 구간) 표본 축소가 6건 났다 — 원정이 8원형에서
// 6원형으로 줄고 상성가성비 봇이 통째로 빠졌다. 그런데 그 판의 실제 소요는
// 468초였다. 1080초 예산 중 7.8분만 쓰고도 「예산이 모자란다」고 판단한 것이다.
// 축소 규칙이 최악치(worstTicks)로 견적을 내는데 실전은 판이 상한 전에 끝나기
// 때문이다. 32분으로 올리면 축소 0건이 되고 실측은 575초(9.6분)다.
// **시계가 아니라 견적이 묶고 있었으므로 예산을 올려도 실제로 더 걸리지 않는다.**
// 조용히 줄어든 표본은 격차를 부풀린다 — 이 구간이 여태 앓던 병과 같은 종류다.
const BUDGET_MS = Math.max(1, +argVal('budget', 32)) * 60000;
...
async function run() {
  ...
  //   무작위    시드 고정 난수
  //
  // **도배를 궁수 하나로 대표시키던 것이 이 계기의 사각지대였다.**
  // 「검보다 강한 쪽으로 잡았다」고 적어 뒀지만 그건 검·궁 둘만 재 본 결론이었다.
  // 여섯 종 전부 세워 보니 진짜 최강 도배는 **기병**이다 (같은 트리 실측):
  //   상성 100%  ·  도배기 60% (+0.153)  ·  순환 60%  ·  도배궁 40% (−0.123)
  //   도배검 20%  ·  도배창 20%  ·  도배거 20%  ·  도배투 0%
  // 궁수 기준 격차는 +60%p 인데 **정직한 격차(기병 기준)는 +40%p** 다.
  // 20%p 를 계기가 만들어 내고 있었다. 계약 §5.5 의 문턱이 딱 40%p 이므로
  // 이 차이는 「여유 있게 통과」와 「문턱에 걸쳐 있음」을 가른다.
  //
  // 최고 도배와 비교하는 규칙(아래 bestSpam)은 원래부터 있었다. 비교 대상이
  // 집합에 없었을 뿐이다 — **최댓값을 고르는 코드는 후보가 하나면 아무 일도 안 한다.**
  const GAP_BOTS = [
    { name: '상성', spec: { pick: 'counter' } },
    { name: '상성가성비', spec: { pick: 'counterGold' } },
    { name: '도배검', spec: { pick: 'spam', k: 0 } },
    { name: '도배창', spec: { pick: 'spam', k: 1 } },
    { name: '도배궁', spec: { pick: 'spam', k: 2 } },
    { name: '도배기', spec: { pick: 'spam', k: 3 } },
    { name: '도배거', spec: { pick: 'spam', k: 4 } },
    { name: '도배투', spec: { pick: 'spam', k: 5 } },
    { name: '순환', spec: { pick: 'cycle' } },
    { name: '무작위', spec: { pick: 'rand', seed: 20260802 } },
  ];
  // 예산 부족 시 이 순서로 버린다. **약한 도배부터 버리고 강한 도배는 끝까지 남긴다** —
  // 격차는 최고 도배로 재므로, 강한 쪽을 버리면 격차가 조용히 부풀려진다.
  // 그게 정확히 이 구간이 여태 앓던 병이다. 도배기·도배궁은 목록에 없다(안 버린다).
  const GAP_BOT_DROP = ['무작위', '상성가성비', '도배투', '도배거', '도배창', '도배검'];
