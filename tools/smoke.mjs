    ...
    decide(t) {
      ...
      if (M === 'safe') {
        ...
        for (let l = 0; l < 3; l++) {
          ...
          // **세 행을 본다.** 두 행만 보면 기둥이 연달아 어긋나게 놓였을 때
          // 지금 고른 레인이 다다음 행에서 막히는 걸 못 본다. 사람은 화면에서
          // 9행 앞까지 보이므로 이 정도는 봐야 사람의 대리인이 된다.
          let c = this.cost(g, row, l) * 100
                + this.cost(g, row + 1, l) * 10
                + this.cost(g, row + 2, l);
          if (l !== g.lane) c += 5;                        // 굳이 안 옮긴다
          // 도박꾼은 **넘을 수 있는** 장애물이 있어도 코인 쪽으로 간다.
          // 기둥(990)만은 못 넘는다. 이게 도박꾼과 자살의 차이다.
          // 비용 저울을 10→100 으로 바꿨으면 성향 가중치도 같이 옮겨야 한다.
          // 안 옮겼더니 도박꾼의 코인 유인(-12)이 자세 장애물(100)에 묻혀
          // 도박꾼이 얌전해졌고, 봇 넷이 전부 BALANCED 로 뭉쳤다.
          // -120 은 넘을 수 있는 장애물(100)은 이기고 기둥(9900)에는 진다.
          if (this.coinNear(g, row, l)) c -= (M === 'greedy' ? 120 : (M === 'play' ? 6 : 0));
      ...
      // **거리가 아니라 시간으로 잡는다.** 170 같은 고정 거리로 두면 속도가
      // 420→760 으로 오르는 동안 이동에 쓸 수 있는 시간이 55% 로 줄어들어,
      // 봇이 보간이 끝나기 전에 행을 만난다. 사람은 거리가 아니라 여유 시간으로
      // 판단하므로 이게 맞는 모델이다. 부스트(1.55배)가 들어오면서 이 차이가
      // 드러났다 — 부스트 자체가 아니라 봇의 모델이 속도에 안 따라간 것이다.
      // **거리가 아니라 시간으로 잡되, 행 간격을 넘지 않게 한다.**
      // 고정 170 으로 두면 속도가 420→760 으로 오르는 동안 이동에 쓸 수 있는
      // 시간이 55% 로 줄어든다. 그런데 시간으로 0.34초를 잡았더니 최고 속도에서
      // 258 단위가 되어 **행 간격 240 을 넘어섰다** — 바로 다음 행이 문제일 때는
      // 조건이 영원히 성립하지 않아 봇이 가만히 서서 기둥에 박았다.
      // 이동은 130ms 면 끝난다. 0.2초면 여유 70ms 가 남고 240 안에 들어온다.
      const spd0 = g.speed > 1 ? g.speed : 1;
      let shiftAt = Math.min(spd0 * (M === 'greedy' ? 0.11 : 0.2), C.ROW_SPACING * 0.8);
      if (M === 'sloppy') shiftAt = Math.min(spd0 * (0.06 + this.jitter(row + 3) * 0.34), C.ROW_SPACING * 0.9);
      let lane = g.lane;
